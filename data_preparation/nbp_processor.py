...
class NRB_Processor():
    ...
    def __init__(self, pair_anno, taskname, na_type, dir_opts, esm=None, sampling_num=1024, neighbors=128, radius=12):
        ...
        self.pair, self.anno = pair_anno.split("\t")
        print(f"{self.pair} begin processing.")
        self.pdb_id, self.protein_chain, self.nucleic_chains = self.pair.split(':') # 1ddl:A:D_E
        ...
        self.pairfile = os.path.join(self.dir_opts["pair_pdb_dir"], taskname, f"{self.pdb_id}_{self.protein_chain}_{self.nucleic_chains}.pdb")
        ...
        self.site, self.point_res_idx = self._get_site_biolip(self.anno)
    ...
    def _get_site_biolip(self, anno): #主要搞不懂的地方
        '''
    ...
    def _get_nucleic_acid_atom(self):
        ...
        if self.na_type == 'RNA':
            ...
            ligand_residue_names = ["A", "R", "N", "D", "C", 'Q', 'E', 'G', 'H', 'I', 'L',' K', 'M', 'F', 'P', 'S', 'T', 'W', 'Y', "V"]
        na_coords = []
        parser = PDBParser(QUIET=True)
        structrue = parser.get_structure(self.pdb_id, self.pairfile)
        model = Selection.unfold_entities(structrue, "M")[0]
        nucleic_chains = self.nucleic_chains.split('_')
    ...
    def get_data(self):
        ...
        pair_prefix = '_'.join(self.pair.split(':'))
        save_path = os.path.join(self.dir_opts['pt_dir'], self.taskname, pair_prefix)
...
if __name__ == "__main__":
    ...
    dir_opts['pt_dir'] = f'data/rna_test_all_128'
    ...
    for line in lines[:]:
        ...
        if not os.path.exists(os.path.join(dir_opts['pt_dir'], taskname, pair)):
            print(pair)
            if line.startswith("6zz6:D:F_G") or line.startswith("6rui:Q:T_U") or line.startswith("5vc8:A:W"):
                continue
